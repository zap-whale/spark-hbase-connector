...
import org.apache.spark.sql.{ColumnName,DataFrame,SparkSession,SQLContext}
...
class SHC extends FunSuite with BeforeAndAfterEach with BeforeAndAfterAll with Logging {
  ...
  private[hbase] def tableName = "table1"

  private[hbase] def columnFamilies: Array[String] = Array.tabulate(9){ x=> s"cf$x"}
